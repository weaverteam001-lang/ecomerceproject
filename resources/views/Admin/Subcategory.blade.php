...
.form-group {
    display: flex;
    gap: 12px;
    flex-wrap: nowrap; /* 👈 same row force */
    align-items: center;
}

/* same height for select + input */
.form-group select,
.form-group input {
...
.form-group select {
    height: 38px;
    padding: 8px 35px 8px 10px; /* 👈 right space for icon */
    border-radius: 6px;
    border: 1px solid #ccc;
    font-size: 13px;
    width: 220px;
...
<div class="page-wrapper">
    ...
    <div class="add-card">
        ...
        <form action="javascript:void(0)" method="post" id="Subcategory" >
            @csrf
             ...
             @foreach($allsubcategory as $data)
                <tr>
                    ...
                    <td>{{$data->subcategory}}</td>
                      <td>
                            @if($data->status == 1)
                                <a href="{{ route('Admin.changeSubCategoryState', ['id' => Crypt::encrypt($data->id)]) }}"
                  ...
                  <td>{{$data->updated_at}}</td>
                    <td>
                        <a href="{{ route('Admin.EditSubcategory', $data->id) }}" class="action-btn btn-edit">Edit</a>

                            <a href="#"
                            class="action-btn btn-delete"
                            onclick="confirmDelete('{{ route('Admin.DeleteSubcategory', $data->id) }}'); return false;">
                            Delete
                            </a>
    ...
    <script src="https://cdn.jsdelivr.net/npm/toastify-js"></script>
      ...
      <script>
        $(document).ready(function() {
            $('#Subcategory').on('submit', function(event) {
                event.preventDefault(); // Prevent default form submission
        
                var url = "{{route('Admin.GetSubcategory')}}";
                $.ajax({
                    url: url,
                    type: 'POST',
                    headers: {
