...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        fetch('/api/Product') // apna API route yaha daalo
            ...
            .then(res => {
                if (res.status) {
                    ...
                    products.forEach(data => {
                        ...
                        let sizes = '';
                        try {
                            let sizeArr = JSON.parse(data.size);
                            sizes = sizeArr.join(', ');
                        } catch (e) {}

                        let colorHTML = '';
                        try {
                            let names = JSON.parse(data.color_name) || [];
                            let codes = JSON.parse(data.color_code) || [];
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="img-box" onclick="showImage('/${data.image}')">
                                    <img src="/${data.image}">
                                </div>
                            </td>

                            <td>
                                <div class="img-box" onclick="showImage('/${data.second_image}')">
                                    <img src="/${data.second_image}">
                                </div>
                            </td>

                            <td>
                                <div class="img-box" onclick="showImage('/${data.third_image}')">
                                    <img src="/${data.third_image}">
                                </div>
                            </td>

                            <td>
                                <a href="/change-status/${data.id}" 
                                   class="btn-status ${data.status == 1 ? 'btn-active' : 'btn-inactive'}">
                            ...
                            <td>
                                <a href="/edit-product/${data.id}" class="action-btn btn-edit">Edit</a>

                                <a href="#" class="action-btn btn-delete"
                                   onclick="confirmDelete('/delete-product/${data.id}')">
                                   Delete
                                </a>
                            </td>
                        </tr>`;
